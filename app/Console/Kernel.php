...
protected function createCrudViews($viewDir, $model, $columns)
{
    ...
    // Fonction pour déterminer le type d'input HTML en fonction du type SQL
    ...
    $listViewContent .= "<td>
        <a href='/{$modelLower}/edit/<?php echo \$item['id']; ?>'>Edit</a> | 
        <form action='/{$modelLower}/delete/<?php echo \$item['id']; ?>' method='POST'>
    ...
    foreach ($columns as $column) {
        ...
        $createViewContent .= "<input type='{$inputType}' name='{$column['Field']}' id='{$column['Field']}'><br>\n";
    }
    ...
    foreach ($columns as $column) {
        ...
        $editViewContent .= "<input type='{$inputType}' name='{$column['Field']}' value='<?php echo \$item['{$column['Field']}']; ?>' id='{$column['Field']}'><br>\n";
    }
